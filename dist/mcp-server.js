...
function validateOptions(options) {
    ...
    for (const [key, value] of Object.entries(options)) {
        ...
        if (!['keepThreshold', 'preserveRecentMessages', 'maxStateTokens', 'maxRequestTokens', 'truncateHeadChars'].includes(key))
            throw new Error('Unknown option: ' + key);
        if (typeof value !== 'number' || !Number.isFinite(value) || value < 0)
            throw new Error('Invalid ' + key);
        if (key === 'keepThreshold') {
            if (value > 1)
                throw new Error('Invalid keepThreshold');
        }
        else if (!Number.isInteger(value) || (key.startsWith('max') && value === 0))
            throw new Error('Invalid ' + key);
    }
}
function schema() {
...
async function callTool(args) {
    ...
    for (const key of Object.keys(args))
        if (!['messages', 'options', 'model'].includes(key))
...
process.stdin.on('data', (chunk) => {
    ...
    if (Buffer.byteLength(input) > 8 * 1024 * 1024) {
        error(null, -32600, 'Input exceeds 8 MiB');
        process.exit(1);
    }
    ...
    for (const line of lines) {
        ...
        try {
            ...
            void handle(request).catch(() => error(request?.id ?? null, -32000, 'Internal error'));
        }
        catch (cause) {
