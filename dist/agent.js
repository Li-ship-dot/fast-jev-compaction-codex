...
export function normalizeAgentMessages(input) {
    ...
    return input.map((value, index) => {
        ...
        if (!item || typeof item.role !== 'string')
            throw new Error(`messages[${index}] must contain a role`);
        const role = item.role === 'assistant' ? 'assistant' : 'user';
        const calls = item.toolUses ?? item.toolCalls ?? item.tool_calls;
        const results = item.toolResults ?? item.tool_results;
        const toolUses = Array.isArray(calls) ? calls.map(normalizeToolUse).filter((call) => Boolean(call)) : [];
        const toolResults = Array.isArray(results) ? results.map(normalizeToolResult).filter((result) => Boolean(result)) : [];
