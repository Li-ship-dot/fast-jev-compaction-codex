...
function flag(value) {
    if (value !== undefined && typeof value !== 'boolean')
        throw new Error('isError must be boolean');
    return value;
}
function text(value) {
...
function alias(item, names) {
    const present = names.filter((name) => item[name] !== undefined);
    if (present.length > 1)
        throw new Error(`Ambiguous fields: ${present.join(', ')}`);
 ...
 */
export function normalizeAgentMessages(input) {
    ...
    const messages = input.map((value) => {
        ...
        if (!['user', 'assistant', 'system', 'developer', 'tool'].includes(String(item.role)))
        ...
        if (item.tool_call_id !== undefined) {
            if (role !== 'tool' || toolResults.length)
                throw new Error('Ambiguous tool result');
        ...
        if (toolResults.length && !['user', 'tool'].includes(role))
            throw new Error('Invalid tool-result role');
        if (role === 'tool' && toolResults.length !== 1)
            throw new Error('Tool message requires one result');
